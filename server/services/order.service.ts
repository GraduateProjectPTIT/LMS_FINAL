...
export const getUserOrdersService = async (
  ...
  // Collect all courseIds from all items in all orders
  const courseIds = [
    ...new Set(
      normalized.flatMap((order: any) =>
        Array.isArray(order.items)
          ? order.items.map((item: any) => String(item.courseId)).filter(Boolean)
          : []
      )
    ),
  ];
  ...
  // Attach course info to each item
  const normalizedWithCourse = normalized.map((order: any) => ({
    ...order,
