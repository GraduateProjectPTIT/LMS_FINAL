...
export default function PostEditForm({ postId }: PostEditFormProps) {
        ...
        const fetchPost = async () => {
            try {
                const res = await fetch(
                    `${process.env.NEXT_PUBLIC_BACKEND_BASEURL}/api/post/${postId}`,
                    {
                        credentials: "include",
                    }
                );
                ...
                if (post.coverImage?.url) {
                    ...
                    setOriginalCoverUrl(post.coverImage.url);
                }
            } catch (error: any) {
    ...
    const handleUploadImage = async (blobInfo: any) => {
        ...
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_BASEURL}/api/post/upload-image`,
                {
                    method: "POST",
                    credentials: "include",
                    body: form,
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            } else if (!coverPreview && originalCoverUrl) {
                // Image was removed
    ...
    if (loading) {
        return (
            <div className="w-full min-h-screen theme-mode flex items-center justify-center">
