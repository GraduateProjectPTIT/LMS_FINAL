...
const RegisterForm = ({
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_BASEURL}/api/auth/register`,
        {
          method: "POST",
          headers: {
  ...
  const handleOAuthLogin = async (provider: "google" | "github") => {
    try {
      ...
      // 1. Lấy callbackUrl gốc từ URL hiện tại
      const originalCallbackUrl = searchParams?.get("callbackUrl") || "/";

      // 2. Tạo URL mới cho NextAuth, đính kèm cả social=1 và callbackUrl gốc
      const nextAuthCallbackUrl = new URL(
        `${window.location.origin}${pathname}`
      );
      nextAuthCallbackUrl.searchParams.set("social", "1");
      nextAuthCallbackUrl.searchParams.set("callbackUrl", originalCallbackUrl);
      ...
      await signIn(provider, {
        callbackUrl: nextAuthCallbackUrl.toString(),
        redirect: true,
      });
    } catch (err) {
          ...
          <div className="flex flex-col items-center justify-center gap-3 mt-4">
            ...
            <div className="w-full">
              ...
              >
                ...
                />
                <div className="mx-[10px]">{field("name").icon}</div>
              </div>
