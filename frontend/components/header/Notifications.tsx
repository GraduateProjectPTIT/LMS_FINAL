...
const Notifications = () => {
    ...
    const fetchNotifications = useCallback(async (status: 'unread' | 'all' = 'unread') => {
        try {
            dispatch(fetchStart());
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_BASEURL}/api/notification/my?status=${status}&page=1&limit=20`,
            ...
            >
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className={`text-sm ${isUnread ? 'font-semibold text-gray-900 dark:text-gray-100' : 'font-normal text-gray-700 dark:text-gray-300'}`}>
                            {n.title}
                        </div>
                        {n.message ? (
